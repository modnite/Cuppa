...
/* CUPPA: temporary diagnostic tracing for the request/response path, independent of the
 * upstream DEBUG_puts/DEBUG_printf machinery (which is compiled out unless -DDEBUG is set
 * and, even then, writes to a file descriptor rather than logcat). */
#include <android/log.h>
#define CUPPA_TRACE(...) __android_log_print(ANDROID_LOG_INFO, "CupsRequestTrace", __VA_ARGS__)
		...
		int        outfile)	/* I - File to write to or -1 for none */
{
  ...
  *
  * CUPPA: this build has no TLS backend compiled in (HAVE_SSL is undefined — see
  * cups/config.h), so cupsGetResponse()'s "reconnect and force encryption" handling for
  * HTTP_STATUS_UPGRADE_REQUIRED (426) is compiled out (#ifdef HAVE_SSL). A printer that
  * requires a secure connection returns 426 on every attempt, and since 426 is deliberately
  * excluded from the break condition below (upstream expects the SSL branch to have already
  * upgraded the connection and be worth retrying), this loop would otherwise spin
  * indefinitely — reproduced live against a real network printer that mandates TLS.
  * io_attempts bounds it so a printer we fundamentally cannot satisfy (no TLS support yet)
  ...
  */
  ...
  while (response == NULL)
  {
    ...
    CUPPA_TRACE("cupsDoIORequest: calling cupsSendRequest...");
    ...
    */
    ...
    CUPPA_TRACE("cupsDoIORequest: cupsSendRequest returned status=%d", (int)status);
    ...
    */
    ...
    {
      CUPPA_TRACE("cupsDoIORequest: calling cupsGetResponse...");
      response = cupsGetResponse(http, resource);
      status   = httpGetStatus(http);
      CUPPA_TRACE("cupsDoIORequest: cupsGetResponse returned response=%p, status=%d", (void*)response, (int)status);
    }
    ...
    {
      CUPPA_TRACE("cupsDoIORequest: breaking out of retry loop, status=%d", (int)status);
    ...
    if (response == NULL)
    {
      if (++io_attempts > 5)
      {
        CUPPA_TRACE("cupsDoIORequest: giving up after %d attempts stuck at status=%d "
                    "(likely HTTP_STATUS_UPGRADE_REQUIRED with no TLS backend available)",
                    io_attempts, (int)status);
        _cupsSetHTTPError(status == HTTP_STATUS_UPGRADE_REQUIRED
                              ? status
                              : HTTP_STATUS_SERVICE_UNAVAILABLE);
        break;
      }
      CUPPA_TRACE("cupsDoIORequest: response still NULL, status=%d does not match break condition — looping again (attempt %d)", (int)status, io_attempts);
    }
		...
		size_t     length)	/* I - Length of data to follow or @code CUPS_LENGTH_VARIABLE@ */
{
  ...
  int			post_failures = 0;
					/* Consecutive httpPost() failures this
					 * call, each followed by a successful
					 * reconnect (CUPPA: bounded below —
					 * unbounded, this was an infinite loop
					 * against any peer that accepts the TCP
					 * connection but then always rejects the
					 * POST, reproduced live against a real
					 * network printer: >1000 reconnects/min). */
  ...
  */
  ...
  for (;;)
  {
    ...
    */
    ...
    CUPPA_TRACE("cupsSendRequest: calling httpPost (attempt %d)...", post_failures + 1);

    {
      int post_result = httpPost(http, resource);
      CUPPA_TRACE("cupsSendRequest: httpPost returned %d (errno=%d %s)", post_result, errno, strerror(errno));
      ...
      if (post_result)
      {
        ...
        if (++post_failures > 3)
        {
          /* CUPPA: give up instead of retrying forever — a peer that accepts
           * the TCP connection but always rejects the POST would otherwise
           * keep this loop reconnecting indefinitely with no delay. */
          DEBUG_puts("1cupsSendRequest: Giving up after repeated POST failures.");
          CUPPA_TRACE("cupsSendRequest: giving up after %d POST failures", post_failures);
          _cupsSetError(IPP_STATUS_ERROR_SERVICE_UNAVAILABLE, strerror(errno), 0);
          return (HTTP_STATUS_SERVICE_UNAVAILABLE);
        }

        CUPPA_TRACE("cupsSendRequest: calling httpReconnect2...");
        if (httpReconnect2(http, 30000, NULL))
        {
          DEBUG_puts("1cupsSendRequest: Unable to reconnect.");
          CUPPA_TRACE("cupsSendRequest: httpReconnect2 failed, giving up");
          return (HTTP_STATUS_SERVICE_UNAVAILABLE);
        }
        CUPPA_TRACE("cupsSendRequest: httpReconnect2 succeeded, retrying POST");
        continue;
      }
    }
    ...
    */
    ...
    CUPPA_TRACE("cupsSendRequest: entering switch with status=%d (UPGRADE_REQUIRED=%d)", (int)status, (int)HTTP_STATUS_UPGRADE_REQUIRED);
    ...
    switch (status)
    {
      ...
      case HTTP_STATUS_ERROR :
          CUPPA_TRACE("cupsSendRequest: hit CONTINUE/OK/ERROR case, returning %d", (int)status);
	  ...
	  */
          ...
          CUPPA_TRACE("cupsSendRequest: hit UPGRADE_REQUIRED case, reconnecting...");
          DEBUG_puts("2cupsSendRequest: Reconnecting after "
	             "HTTP_STATUS_UPGRADE_REQUIRED.");

	  if (httpReconnect2(http, 30000, NULL))
	  {
	    CUPPA_TRACE("cupsSendRequest: httpReconnect2 failed (errno=%d %s)", errno, strerror(errno));
	    DEBUG_puts("1cupsSendRequest: Unable to reconnect.");
	    return (HTTP_STATUS_SERVICE_UNAVAILABLE);
	  }

	  CUPPA_TRACE("cupsSendRequest: reconnected, calling httpEncryption(HTTP_ENCRYPTION_REQUIRED)...");
          DEBUG_puts("2cupsSendRequest: Upgrading to TLS.");
	  if (httpEncryption(http, HTTP_ENCRYPTION_REQUIRED))
	  {
	    CUPPA_TRACE("cupsSendRequest: httpEncryption failed: %s", cupsLastErrorString());
	    DEBUG_puts("1cupsSendRequest: Unable to encrypt connection.");
	    return (HTTP_STATUS_SERVICE_UNAVAILABLE);
	  }
	  CUPPA_TRACE("cupsSendRequest: httpEncryption succeeded, retrying POST over TLS");
	  break;
#endif /* HAVE_SSL */
	  ...
	  */
	  ...
	  CUPPA_TRACE("cupsSendRequest: hit default case for status=%d, returning as-is", (int)status);
	  return (status);
    }
  }
}
