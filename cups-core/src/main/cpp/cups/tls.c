...
#  elif defined(HAVE_OPENSSL)
     /* CUPPA: real TLS client support for Android, backed by a statically-linked OpenSSL 1.1.1
      * built for each ABI (see third_party/openssl/ and CMakeLists.txt). tls-boringssl.c's API
      * usage (BIO_meth_new, SSL_CTX_set_min/max_proto_version, etc.) is OpenSSL 1.1.0+ standard,
      * not BoringSSL-specific, so it works unmodified against real OpenSSL. */
#    include "tls-boringssl.c"
#  endif /* HAVE_GNUTLS */
#else
