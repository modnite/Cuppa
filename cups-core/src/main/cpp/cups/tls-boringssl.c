...
/*
 * CUPPA: tls_min_version/tls_max_version are CUPS's own small internal enum values
 * (_HTTP_TLS_SSL3=0 .. _HTTP_TLS_1_3=4, _HTTP_TLS_MAX=5 meaning "no upper limit"), set via
 * _httpTLSSetOptions() from usersys.c's _cupsSetDefaults(). SSL_CTX_set_min/max_proto_version()
 * expect real OpenSSL protocol version constants (e.g. 0x0303 for TLS 1.2), not these small
 * integers — passing them through unmodified (as the original AOSP file did) would hand the
 * library meaningless version numbers and could reject every handshake. Translate here.
 */
static int
cuppa_tls_proto_version(int http_tls_version, int is_max)
{
  switch (http_tls_version)
  {
    case _HTTP_TLS_SSL3: return SSL3_VERSION;
    case _HTTP_TLS_1_0:  return TLS1_VERSION;
    case _HTTP_TLS_1_1:  return TLS1_1_VERSION;
    case _HTTP_TLS_1_2:  return TLS1_2_VERSION;
    case _HTTP_TLS_1_3:  return TLS1_3_VERSION;
    default:             return is_max ? 0 /* no upper limit */ : TLS1_VERSION;
  }
}
...
_httpTLSStart(http_t *http)		/* I - Connection to server */
{
  ...
  SSL_CTX_set_min_proto_version(context, cuppa_tls_proto_version(tls_min_version, 0));
  SSL_CTX_set_max_proto_version(context, cuppa_tls_proto_version(tls_max_version, 1));
